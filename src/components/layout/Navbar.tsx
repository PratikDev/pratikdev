...
export function Navbar({ mode, onModeChange }: NavbarProps) {
	return (
		<header className="fixed inset-x-0 top-0 z-50 border-b border-border bg-background/90 backdrop-blur transition-mode">
			<div className="mx-auto flex min-h-[var(--site-nav-height)] w-full max-w-6xl flex-wrap items-center justify-between gap-x-4 gap-y-2 px-4 py-3 sm:px-6 lg:px-8">
				<a
					href="#top"
				...
				<nav
					...
					className="order-3 flex w-full gap-1 overflow-x-auto pb-1 text-xs sm:order-none sm:w-auto sm:pb-0"
				>
					{navItems.map((item) => (
						<a
							key={item.id}
